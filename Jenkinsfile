pipeline {
  ...
  environment {
    ...
    HTTP_PORT = "${18080 + (env.BUILD_NUMBER as Integer) % 1000}"
    NODE_ENV = "ci"
    CI = "true"
  }
  ...
  stages {
    stage('CI') {
      ...
      stages {
        ...
        stage('Docker Build (target=prod)') {
          steps {
            sh '''
              set -e
              test -f Dockerfile
        ...
        stage('Analyze (compose.analyze.yaml)') {
          ...
          steps {
            sh '''
              ...
              $COMPOSE -f compose.analyze.yaml up -d --build analyze

              echo "Sleeping 10s to let analyzer start..."
              sleep 10

              echo "Analyze logs:"
              $COMPOSE -f compose.analyze.yaml logs --no-color --tail=200 analyze || true
              $COMPOSE -f compose.analyze.yaml down -v || true
          ...
          post {
            always {
              archiveArtifacts artifacts: '**/*report*.html, **/*stats*.json, reports/**', allowEmptyArchive: true
            }
          }
        }
      }
